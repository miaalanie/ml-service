...
def _hitung_usia(tanggallahir) -> int | None:
    ...
    if isinstance(tanggallahir, str):
        try:
            tanggallahir = datetime.strptime(str(tanggallahir)[:10], "%Y-%m-%d").date()
        except ValueError:
            return None
    today = date.today()
    return (
...
class BiodataValidator:
    def validate(
        ...
        req_gender = (
            None
            if lowongan.preferensi_gender == "Semua"
            else lowongan.preferensi_gender
        )
        min_usia = (
            lowongan.usia_min if lowongan.usia_min and lowongan.usia_min > 0 else None
        )
        max_usia = (
            lowongan.usia_max if lowongan.usia_max and lowongan.usia_max > 0 else None
        )
        min_edu_kode = (
            lowongan.minimal_pendidikan.kode if lowongan.minimal_pendidikan else 0
        )
        ...
        if log_detail:
            logger.info(
                "  [BIODATA] Syarat loker: gender=%s, usia=%s-%s, min_edu_level=%s, min_exp=%s bulan",
                req_gender or "bebas",
                min_usia or "-",
                max_usia or "-",
                min_edu_req,
        ...
        if usia is None:
            ...
            if too_young:
                ...
                result["usia_note"] = (
                    f"Usia pelamar {usia} tahun di bawah batas minimum {min_usia} tahun yang disyaratkan loker"
                )
            elif too_old:
                result["usia_match"] = False
                result["usia_note"] = (
                    f"Usia pelamar {usia} tahun melebihi batas maksimal {max_usia} tahun yang disyaratkan loker"
                )
            else:
        ...
        if log_detail:
            logger.info(
                "  [BIODATA] Usia pelamar=%s -> match=%s (%s)",
                usia,
                result["usia_match"],
                result["usia_note"],
            )
        ...
        # --- 2. GENDER ---
        pelamar_gender = _normalize_gender(pelamar_biodata.get("jeniskelamin", ""))
        ...
        if req_gender is None:
            ...
            result["gender_note"] = (
                f"Jenis kelamin pelamar ({pelamar_gender}) sesuai dengan syarat loker"
            )
        else:
            result["gender_match"] = False
            result["gender_note"] = (
                f"Loker mensyaratkan {req_gender}, pelamar berjenis kelamin {pelamar_gender}"
            )

        if log_detail:
            logger.info(
                "  [BIODATA] Gender pelamar=%s, syarat loker=%s -> match=%s (%s)",
                pelamar_gender,
                req_gender or "bebas",
                result["gender_match"],
                result["gender_note"],
        ...
        if min_edu_req == 0:
            ...
            if pelamar_edu_level >= min_edu_req:
                result["edu_gap"] = False
                result["edu_note"] = (
                    f"Pendidikan pelamar ({pelamar_edu_kategori}) memenuhi syarat minimum loker ({req_label})"
                )
            else:
                result["edu_gap"] = True
                result["edu_note"] = (
                    f"Pendidikan pelamar ({pelamar_edu_kategori}) di bawah syarat minimum loker ({req_label})"
                )

        if log_detail:
            logger.info(
                "  [BIODATA] Edu gap=%s (%s)", result["edu_gap"], result["edu_note"]
            )
        ...
        if min_exp_bulan == 0:
            ...
            if total_pengalaman_bulan >= min_exp_bulan:
                result["exp_gap"] = False
                result["exp_note"] = (
                    f"Total pengalaman pelamar ({total_pengalaman_bulan} bulan) memenuhi syarat minimum loker ({min_exp_bulan} bulan)"
                )
            else:
                result["exp_gap"] = True
                result["exp_note"] = (
                    f"Total pengalaman pelamar ({total_pengalaman_bulan} bulan) kurang dari syarat minimum loker ({min_exp_bulan} bulan)"
                )

        if log_detail:
            logger.info(
                "  [BIODATA] Exp gap=%s (%s)", result["exp_gap"], result["exp_note"]
            )

        # --- 5. BAHASA ---
        skill_loker_text = " ".join(s.nama.lower() for s in (lowongan.skills or []))
        ...
        if not lang_found and not lang_missing:
            ...
            result["lang_note"] = (
                f"Pelamar memiliki kemampuan bahasa yang dibutuhkan loker: {', '.join(lang_found)}"
            )
        else:
            result["lang_note"] = (
                f"Bahasa yang dibutuhkan loker namun tidak ditemukan di profil pelamar: {', '.join(lang_missing)}"
            )

        if log_detail:
            logger.info(
                "  [BIODATA] Bahasa: ditemukan=%s, kurang=%s (%s)",
                lang_found,
                lang_missing,
                result["lang_note"],
            )
